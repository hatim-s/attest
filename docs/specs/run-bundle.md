...
| --------------------- | --------------- | ---------------------------- | -------------------------------------------------------------------- |
| First                 | `bundle_header` | `bundle_version`, `run`      | Declares `attest.bundle` and embeds a `RunRecord`-shaped JSON value. |
...
The bundle schema is `attest.bundle`. Writers emit only this schema until a new, explicitly documented schema is introduced. Readers MUST spool and verify the entire bundle before exposing any records to callers. They MUST verify the first and only header, its known schema, case record shapes, the final footer, `case_count`, and `content_hash`.
