...
CLI: `printf '%s' '{"protocol":"attest.agent-invocation","run_id":"01ARZ3NDEKTSV4RRFFQ69G5FAV","case_id":"demo","input":{}}' | node conformance/src/_tests_/fixtures/fake-agents/cli-agent.cjs --behavior=happy`

HTTP: `PORT=3000 node conformance/src/_tests_/fixtures/fake-agents/http-agent.cjs`, then `curl -X POST http://127.0.0.1:3000/happy -H 'content-type: application/json' -d '{"protocol":"attest.agent-invocation","run_id":"01ARZ3NDEKTSV4RRFFQ69G5FAV","case_id":"demo","input":{}}'`
...
| slow-drip               | CLI, HTTP              | deadline enforcement during progress; default 100ms drip completes a happy envelope in ~6.5s, so runner timeout tests must set `AGENT_DRIP_MS` relative to their configured deadline | `AGENT_DRIP_MS`         |
