...
import { relativePathSchema, resourceIdSchema, sha256Schema } from './v2-shared.js';
import { CASE_SCHEMA_VERSION, DATASET_SCHEMA_VERSION } from './versions.js';
...
/** Encodes metadata for one canonical, ordered v2 dataset JSONL file. */
const datasetResourceSchema = z
  .strictObject({
    schema: z.literal(DATASET_SCHEMA_VERSION),
    case_schema: z.literal(CASE_SCHEMA_VERSION),
    id: resourceIdSchema,
    name: z.string().min(1),
    case_count: z.number().int().nonnegative(),
    provenance: datasetImportProvenanceSchema.optional(),
  })
  .meta({ id: 'V2DatasetResource' });
