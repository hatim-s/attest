...
describe('WebSocket fake server correlation scenarios', () => {
  it('serves serial acknowledgements and results in request order', async () => {
    ...
    const client = await connect(server.url, 'attest.fixture.v1');
    ...
    expect(server.events()[0]?.headers?.['sec-websocket-protocol']).toBe('attest.fixture.v1');
  });
