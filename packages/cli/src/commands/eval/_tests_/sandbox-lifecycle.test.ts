...
vi.mock('@attest/core', async (importOriginal) => ({
  ...(await importOriginal<typeof import('@attest/core')>()),
...
const createRun = async (execution: EvalRun['effective_command']['resolved']['execution']) => {
  const fixture = await readFile(
    new URL('../../../../../contracts/src/_tests/fixtures/eval-run/eval-run.json', import.meta.url),
    'utf8',
  );
