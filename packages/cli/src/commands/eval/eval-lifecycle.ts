...
const resolveHookArgv = (projectRoot: string, argv: readonly string[]): string[] =>
  argv.map((argument) => {
    ...
    if (!isProjectPath(projectRoot, resolved)) {
      throw new Error('Eval hook argument path escapes the project.');
    }
    return resolved;
  });
...
const resolveWorkerDirectory = (
  ...
  if (directory === resolve(projectRoot) || !isProjectPath(projectRoot, directory)) {
    throw new Error(`Eval worker directory escapes the project: ${relative}`);
  }
  return directory;
};
...
const prepareWorkerDirectory = async (projectRoot: string, directory: string): Promise<string> => {
  ...
  while (true) {
    try {
      ...
      if (parent === ancestor) throw new Error('Eval worker directory has no existing ancestor.');
      ancestor = parent;
    }
  }
  if (!isProjectPath(resolvedRoot, ancestor)) {
    throw new Error('Eval worker directory escapes the project through a symbolic link.');
  }
