...
const parseArgvJson = (value: string): string[] => {
  ...
  try {
    ...
    throw new LocalError('cli_usage', '`--argv-json` is not valid JSON.', {
      path: '--argv-json',
...
const parseSandboxJson = (value: string): VercelSandbox => {
  ...
  try {
    ...
    throw new LocalError('cli_usage', '`--sandbox-json` is not valid JSON.', {
      path: '--sandbox-json',
...
const parseJsonValues = (values: readonly string[], path: string): JsonValue[] =>
  values.map((value) => {
    try {
      ...
      throw new LocalError('cli_usage', `${path} must contain valid JSON values.`, {
        path,
...
const parseRequestTemplate = (value: string): Record<string, JsonValue> => {
  ...
  try {
    ...
    throw new LocalError('cli_usage', '`--request-template` is not valid JSON.', {
