...
const readTextSource = async (
  ...
  try {
    ...
    throw new LocalError('cli_usage', 'Could not read the requested metric input.', {
      path,
...
const parseJson = (text: string, path: string, hint: string): unknown => {
  try {
    ...
    throw new LocalError('cli_usage', `${path} is not valid JSON.`, {
      path,
      hint,
      cause: error,
    });
...
const validateMetricCommandRequest = <TCommand extends CommandRequest['command']>(
  ...
  if (!parsed.success) {
    throw new LocalError('cli_usage', 'The metric command request does not match its schema.', {
