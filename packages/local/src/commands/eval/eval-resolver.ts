...
const positiveInteger = (value: number | undefined, fallback: number, label: string): number => {
  ...
  if (!Number.isInteger(resolved) || resolved <= 0) {
    throw new LocalError('cli_usage', `${label} must be a positive integer.`);
  }
  return resolved;
};
...
const resolveEvalRun = (
  ...
  if (
    ...
    throw new LocalError('project_changed', 'The project changed before eval resolution.', {
      details: {
  ...
  if (options.argv.length === 0) {
    throw new LocalError('cli_usage', 'Effective eval command metadata requires non-empty argv.');
  }
  ...
  if (selectedTestIds.length === 0) {
    throw new LocalError('resource_not_found', 'No tests are defined for `--all` selection.', {
      details: { resource_type: 'test' },
    });
  }
  const missingTestIds = selectedTestIds.filter((id) => !testsById.has(id));
  if (missingTestIds.length > 0) {
    throw new LocalError('resource_not_found', 'One or more selected test ids do not exist.', {
